...
public class ReportsController : ControllerBase
{
    ...
    {
        var rows = await _reportService.FarmerReportAsync(filter);
        var headers = new[] { "Farmer", "Weight (kg)", "Boxes", "Cartons", "Sales (₪)", "Commission (₪)", "Net Due (₪)", "Paid (₪)", "Remaining (₪)", "Last Invoice" };
        var body = rows.Select(r => new[]
        {
            r.FarmerName, r.TotalWeightKg.ToString("0.###"), r.TotalBoxes.ToString("0.###"), r.TotalCartons.ToString("0.###"),
    ...
    {
        var rows = await _reportService.MerchantReportAsync(filter);
        var headers = new[] { "Merchant", "Weight (kg)", "Boxes", "Cartons", "Purchases (₪)", "Wood (₪)", "Box Fee (₪)", "Grand Total (₪)", "Paid (₪)", "Remaining (₪)", "Last Invoice" };
        var body = rows.Select(r => new[]
        {
            r.MerchantName, r.TotalWeightKg.ToString("0.###"), r.TotalBoxes.ToString("0.###"), r.TotalCartons.ToString("0.###"),
    ...
    public async Task<IActionResult> DriversPdf([FromQuery] ReportFilterRequest filter)
    {
        ...
        var body = rows.Select(r => new[]
        {
            r.DriverName, r.TotalBoxes.ToString("0.###"), r.TotalCartons.ToString("0.###"), r.TotalTransportFee.ToString("0.##"),
            r.TotalPaid.ToString("0.##"), r.Remaining.ToString("0.##"),
            r.LastInvoiceDate?.ToString("yyyy-MM-dd") ?? "-"
        });
