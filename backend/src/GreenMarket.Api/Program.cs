...
builder.Services.AddScoped<ISackService, SackService>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    // Sacks gained kinds — a colour or a shape per movement — and an existing database has
    // neither the table nor the column. EnsureCreatedAsync only ever builds from the current
    // model on a BRAND-NEW database, so both are added here by hand.
    //
    // "SackKindId" is nullable and stays nullable. Every sack movement recorded before this has
    // none, and those rows are still sacks somebody is holding: they count under "بدون نوع"
    // rather than being dropped or quietly assigned to a colour nobody chose.
    try
    {
        await db.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS sack_kinds (
            ...
            CREATE INDEX IF NOT EXISTS "IX_container_movements_SackKindId"
                ON container_movements ("SackKindId");
            """);
    }
    catch (Exception ex)
    {
    ...
    //
    // A failure here signs everybody out and keeps them out: LiveUserStateMiddleware refuses any
    // request whose session cannot be confirmed live, which is the correct direction to fail but a
    // loud one, so it says exactly that in the log.
    try
    {
        await db.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS user_sessions (
                ...
                -- SHA-256 as hex, both of them. The token itself is never stored.
                "TokenHash"         character varying(64) NOT NULL,
                "PreviousTokenHash" character varying(64) NULL,
                "CreatedAt"         timestamp with time zone NOT NULL DEFAULT now(),
                "LastUsedAt"        timestamp with time zone NOT NULL DEFAULT now(),
                -- Null means never. A session ends because somebody ended it.
                "ExpiresAt"         timestamp with time zone NULL,
                "RevokedAt"         timestamp with time zone NULL,
                "RevokedByUserId"   integer NULL,
                "RevokedReason"     character varying(40) NULL,
                "UserAgent"         character varying(300) NULL
            );
            CREATE INDEX IF NOT EXISTS "IX_user_sessions_TokenHash" ON user_sessions ("TokenHash");
            CREATE INDEX IF NOT EXISTS "IX_user_sessions_PreviousTokenHash" ON user_sessions ("PreviousTokenHash");
            CREATE INDEX IF NOT EXISTS "IX_user_sessions_UserId" ON user_sessions ("UserId");
            """);
    }
    catch (Exception ex)
    {
        app.Logger.LogError(ex, "Failed to prepare the user_sessions table — NOBODY will be able to sign in until this is fixed.");
    }

    // Phone notifications are new, so an existing database has no table for the devices that
    // agreed to them. Nothing else depends on it: a failure here costs notifications and leaves
    // every other screen working, which is why it gets its own try/catch like the rest.
    try
    {
        await db.Database.ExecuteSqlRawAsync("""
            CREATE TABLE IF NOT EXISTS push_subscriptions (
                "Id"          integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                "UserId"      integer NOT NULL REFERENCES users ("Id") ON DELETE CASCADE,
                -- No length cap: Apple's endpoints run past 300 characters and the spec sets no
                -- upper bound, so guessing one here would silently truncate a real subscription.
                ...
                -- The day this device was last told. What stops a restart or a second instance
                -- from delivering the same morning twice.
                "LastDailyOn" date NULL
            );
            CREATE UNIQUE INDEX IF NOT EXISTS "IX_push_subscriptions_Endpoint"
                ON push_subscriptions ("Endpoint");
            CREATE INDEX IF NOT EXISTS "IX_push_subscriptions_UserId"
                ON push_subscriptions ("UserId");
            """);
    }
    catch (Exception ex)
    {
        app.Logger.LogError(ex, "Failed to prepare the push_subscriptions table — phone notifications will not work until this is fixed.");
    }
