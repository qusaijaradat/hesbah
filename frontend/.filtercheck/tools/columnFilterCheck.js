 ...
 */
import { activeFilterCount, applyColumnFilters, derivedOptions, isFilterEmpty, normalizeArabic, } from "../src/lib/columnFilters.js";
let passed = 0;
let failed = 0;
...
const f = (state) => applyColumnFilters(rows, SPECS, state).map((r) => r.id);
...
check("declared options win over the rows", derivedOptions(rows, { key: "x", kind: "select", value: () => "", options: [{ value: "a", label: "A" }] }).length === 1);
console.log("\n== number range ==");
...
check("an impossible combination returns nothing", f({ type: { a: "بائع" }, balance: { a: "5000" } }).length === 0);
check("a blank filter beside a real one is ignored", f({ type: { a: "مشتري" }, name: { a: "" } }).join() === "1,4");
